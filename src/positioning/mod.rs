...
pub async fn positioning_task() {
    ...
    let mut fusion_proc = fusion::SensorFusion::new(0.95);
    ...
    loop {
        ...
        while let Ok(data) = MPU_CHANNEL.try_receive() {
            mpu_delta = mpu_proc.update(data.accel, data.gyro, data.mag);
            updated = true;
        }
        ...
        if updated {
            let state = fusion_proc.update(odom_delta, mpu_delta);
